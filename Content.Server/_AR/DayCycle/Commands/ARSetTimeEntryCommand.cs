...
public sealed class ARSetTimeEntryCommand : LocalizedCommands
{
    private const string Name = "ar-settimeentry";
    ...
    public override CompletionResult GetCompletion(IConsoleShell shell, string[] args)
    {
        ...
        {
            case 1:
                return CompletionResult.FromOptions(CompletionHelper.Components<ARDayCycleComponent>(args[0], entityManager));

            case 2:
                if (!NetEntity.TryParse(args[0], out var mapUid))
                    return CompletionResult.Empty;

                if (!entityManager.TryGetComponent<ARDayCycleComponent>(entityManager.GetEntity(mapUid), out var component))
                    return CompletionResult.Empty;
